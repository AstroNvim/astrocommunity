...
local function get_lsp_rooter()
  if not lsp_rooter_cache then
    lsp_rooter_cache = require("astrocore.rooter").resolve("lsp", {
      ignore = {
        servers = function(client) return not vim.tbl_contains(lsps, client.name) end,
      },
    })
  end
...
local function has_prettier(bufnr)
  ...
  -- Combine current working directory with LSP roots for package.json search
  ...
  for _, root in ipairs(search_roots) do
    ...
    -- Check if package.json exists before trying to decode
...
local null_ls_formatter = function(params)
  ...
  return true -- Allow other null-ls sources for non-JS/TS files or if this is not the primary condition
end
...
return {
  {
    ...
    opts = function(_, opts)
      ...
      opts.handlers.prettierd = function(source_name, methods)
        ...
        if
          null_ls
          and null_ls.builtins
          and null_ls.builtins[methods[1]]
          and null_ls.builtins[methods[1]][source_name]
        then
